...
Both rules are enforced in CI by the [`rmartz/repo-hygiene-action`](https://github.com/rmartz/repo-hygiene-action) action (`.github/workflows/repo-hygiene.yml`): the **`okf`** check validates frontmatter on every non-reserved page, and the **`okf-index`** check validates index reachability. Per-check settings (the OKF `type` vocabulary, docs roots) live in `.repo-hygiene.yml`.
